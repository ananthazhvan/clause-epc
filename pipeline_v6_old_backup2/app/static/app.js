...
function wireCopilot() {
  ...
  const hist = [];
  const md = (s) => esc(s).replace(/\*\*([^*]+)\*\*/g, "<b>$1</b>").replace(/`([^`]+)`/g, '<span class="mono">$1</span>').replace(/\n/g, "<br>");
  ...
  $("#cp-form").onsubmit = async (e) => {
    ...
    if (!q) return;
    inp.value = "";
    log.insertAdjacentHTML("beforeend", '<div class="cp-msg cp-user">' + esc(q) + "</div>");
    const wait = document.createElement("div");
    wait.className = "cp-msg cp-wait";
    wait.textContent = "reading the ledger\u2026";
    log.appendChild(wait);
    log.scrollTop = log.scrollHeight;
    try {
      const r = await postJson("/api/agent", { message: q, history: hist.slice(-8) });
      wait.remove();
      (r.steps || []).forEach((s) => log.insertAdjacentHTML("beforeend", '<div class="cp-step mono">\u2192 ' + esc(s.tool) + "(" + esc(s.args || "") + ")" + (s.note ? " \u00b7 " + esc(s.note) : "") + "</div>"));
      log.insertAdjacentHTML("beforeend", '<div class="cp-msg cp-ai">' + md(r.reply || "") + "</div>");
      hist.push({ role: "user", content: q }, { role: "assistant", content: r.reply || "" });
    } catch (err) {
      wait.remove();
      log.insertAdjacentHTML("beforeend", '<div class="cp-msg cp-err">' + esc(err.message || String(err)) + "</div>");
    }
    log.scrollTop = log.scrollHeight;
  };
}
