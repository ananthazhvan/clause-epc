...
- **Schedules** live in Primavera P6 / MS Project; teams exchange XER / XML /
  ...
  pipeline uses (activity, duration, predecessors, float). Native XER import
  is roadmap — same slot, different reader.
- **Procurement**: expediting reports (Procore, SAP, 4castplus) track each PO
  ...
  `po_register.csv` is the core of that report. Extra columns (e.g.
  `dispatch_date`, `eta`, `current_location`) are accepted and preserved.
