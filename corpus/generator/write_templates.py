...
<div class="page">
    ...
    {% if section == "26 33 53" %}
    <h3>2.3 Component Description</h3>
    <p>A. Static Bypass Switch: The UPS internal static bypass switch shall be a solid-state, high-speed electronic switch configured for zero-break transfers. 
       <strong>bypass_transfer_time:</strong> Switch transfer time from inverter to bypass source under fault conditions shall not exceed 4.0 ms.</p>
    <p>B. Overload Capacity: The inverter circuit shall be capable of sustaining clear overload conditions without dropping load. 
       <strong>overload_rating:</strong> Inverter shall support a minimum overload threshold of 125% for up to 10 minutes continuously before transferring to bypass.</p>

    <h3>2.4 Enclosures and Finishes</h3>
    <p>A. Airflow Architecture: The cabinet structural ventilation configuration must align with hot/cold aisle containment systems. 
       <strong>airflow_configuration:</strong> Equipment frame cooling channels shall utilize a front-inlet and top-hood exhaust airflow configuration.</p>
    <p>B. Enclosure Finishing: Structural steel panel surfaces must undergo automated multi-stage washing, rust-inhibiting phosphating, and powder paint baking. 
       <strong>paint_finish:</strong> UPS structural enclosure paint finish shall be specified as standard RAL 7035 (Light Grey) polyester powder coating.</p>
    {% endif %}
