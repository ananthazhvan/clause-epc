...
def main():
    ...
    def registers_all():
        return have("schedule.csv") and have("po_register.csv") and (have("cx_test_register.csv") or have("cx_register.csv"))

    stages = [
        dict(id="S1", label="parse every uploaded document", llm=False, core=True,
             cmd=[py, "-u", "m1_parse.py", "--corpus", corpus, "--out", out, "--allow-llm"],
             need=lambda: True, why=""),
        dict(id="S2", label="compile specification clauses into checkable rules", llm=True, core=True,
             cmd=[py, "-u", "m2_rules.py", "--all", "--out", out],
             need=spec_parsed, why="no specification recognized in the upload"),
        dict(id="S3", label="extract vendor claims from submittal packages", llm=True, core=True,
             cmd=[py, "-u", "m3_claims.py", "--all", "--out", out],
             need=subs_parsed, why="no submittal transmittal recognized in the upload"),
        dict(id="S4", label="verify every claim against every rule", llm=False, core=True,
             cmd=[py, "-u", "m4_verify.py", "--all", "--out", out],
             need=claims_exist, why="no vendor claims extracted"),
        dict(id="S5", label="adjudicate unresolved verdicts against full package text", llm=True, core=True,
             cmd=[py, "-u", "m4b_adjudicate.py", "--out", out],
             need=claims_exist, why="no vendor claims extracted"),
        dict(id="S6", label="apply addenda in date order + blast wave", llm=False, core=True,
             cmd=[py, "-u", "m5_addendum.py"], need=lambda: True, why=""),
        dict(id="S6b", label="cross-evidence conflict sweep - contradictions routed to a human", llm=False, core=True,
             cmd=[py, "-u", "m17_conflicts.py", "--out", out],
             need=claims_exist, why="no vendor claims extracted"),
        dict(id="S7", label="dispositions + NCR register", llm=False, core=False,
        ...
        dict(id="S10", label="paperwork drafts + specification self-lint", llm=False, core=False,
             cmd=[py, "-u", "m10_paperwork.py"], need=lambda: True, why=""),
        dict(id="S11", label="commissioning readiness packs", llm=False, core=False,
             cmd=[py, "-u", "m11_cx.py"], need=lambda: have("cx_test_register.csv") or have("cx_register.csv"),
             why="commissioning test register not uploaded"),
        dict(id="S12", label="compile the ontology - objects, links, money, rollups, certification evidence", llm=False, core=False,
