...
def main():
    ...
    # ---------------- ACC field logs (worklog / equipment) -----------------
    for p in glob.glob(os.path.join(corpus, "**", "*.json"), recursive=True):
        if "_answer_key" in p:
            continue
        d = jload(p)
        if not isinstance(d, dict):
            continue
        for w in d.get("worklogEntries") or []:
            trade = (w.get("trade") or "").strip()
            if not trade:
                continue
            tid = f"trade:{trade}"
            planned, actual = w.get("plannedHeadcount"), w.get("headcount")
            O.obj(tid, "trade", f"{trade} crew",
                  props={"headcount": actual, "planned_headcount": planned,
                         "hours_logged": round((w.get("timespan") or 0) / 3600000.0, 1),
                         "log_date": w.get("date")})
            act = (w.get("activity") or "").strip()
            if act and f"act:{act}" in O.objects:
                O.link(tid, f"act:{act}", "works_on")
            try:
                if planned and actual is not None and float(actual) < 0.8 * float(planned):
                    O.insight(tid, f"Under-manned: {actual} of {planned} planned {trade} workers logged - productivity factor {round(float(actual)/float(planned),2)}.", "warn")
                    if act and f"act:{act}" in O.objects:
                        O.insight(f"act:{act}", f"{trade} crew at {actual}/{planned} of plan - float erosion risk on this activity.", "warn")
            except (TypeError, ValueError):
                pass
        for w in d.get("equipmentEntries") or []:
            item = (w.get("item") or "").strip()
            if not item:
                continue
            O.obj(f"equip:{item}", "equipment", item,
                  props={"units": w.get("quantity"),
                         "hours_run": round((w.get("timespan") or 0) / 3600000.0, 1),
                         "log_date": w.get("date")})
            act = (w.get("activity") or "").strip()
            if act and f"act:{act}" in O.objects:
                O.link(f"equip:{item}", f"act:{act}", "deployed_on")

    # ---------------- Hexagon Smart Materials (BOM vs PMS class) -----------
    for p in glob.glob(os.path.join(corpus, "**", "*.json"), recursive=True):
        if "_answer_key" in p:
            continue
        d = jload(p)
        if not isinstance(d, dict) or not isinstance(d.get("bomLines"), list):
            continue
        pms = {c.get("pms_id"): c for c in d.get("pmsClasses") or [] if isinstance(c, dict)}
        for b in d["bomLines"]:
            bid = (b.get("bom_id") or "").strip()
            if not bid:
                continue
            mid = f"mat:{bid}"
            cls = pms.get(b.get("pms_id")) or {}
            bad_sched = bad_mat = False
            want_sch = str(cls.get("required_schedule") or "").strip().lower()
            got_sch = str(b.get("wall_thickness_schedule") or "").strip().lower()
            bad_sched = bool(want_sch and got_sch and want_sch != got_sch)
            want_mat = str(cls.get("required_material") or "").strip().lower()
            got_mat = str(b.get("asme_material_standard") or "").strip().lower()
            bad_mat = bool(want_mat and got_mat and want_mat not in got_mat and got_mat not in want_mat)
            O.obj(mid, "material", f"{bid} \u00b7 {str(b.get('description') or b.get('part_number') or '')[:60]}",
                  status="NONCONFORMING" if (bad_sched or bad_mat) else "",
                  props={"part_number": b.get("part_number"), "line_number": b.get("line_number"),
                         "nominal_size": b.get("nominal_size"), "schedule": b.get("wall_thickness_schedule"),
                         "material_standard": b.get("asme_material_standard"),
                         "heat_number": b.get("heat_number"), "mill_cert": b.get("mill_cert_id"),
                         "pms_class": b.get("pms_id"), "service": cls.get("service")})
            if bad_sched:
                O.insight(mid, f"PMS class {b.get('pms_id')} ({cls.get('service')}) requires {cls.get('required_schedule')} but this line is cut as {b.get('wall_thickness_schedule')} - fabrication hold.", "bad")
            if bad_mat:
                O.insight(mid, f"Material grade {b.get('asme_material_standard')} does not satisfy PMS requirement {cls.get('required_material')}.", "bad")
            if not b.get("mill_cert_id"):
                O.insight(mid, "No mill test certificate on file - heat-number traceability gap.", "warn")
            oid = po_by_digits.get(digits(b.get("linked_po")))
            if oid:
                O.link(mid, oid, "fabricated_under")
                if bad_sched or bad_mat:
                    O.insight(oid, f"BOM line {bid} nonconforming against its piping class - fabrication hold risk.", "warn")
            pkg = (b.get("linked_package") or "").strip()
            if pkg and f"package:{pkg}" in O.objects:
                O.link(mid, f"package:{pkg}", "supports")

    # ---------------- Aconex-style document register + workflows -----------
    for p in glob.glob(os.path.join(corpus, "**", "*.json"), recursive=True):
        if "_answer_key" in p:
            continue
        d = jload(p)
        if not isinstance(d, dict) or not isinstance(d.get("documents"), list):
            continue
        for doc in d["documents"]:
            numb = (doc.get("document_number") or "").strip()
            if not numb:
                continue
            did = f"doc:{numb}"
            O.obj(did, "doc", f"{numb} \u00b7 {str(doc.get('title') or '')[:55]}",
                  status=(doc.get("revision_status") or "").upper().replace(" ", "_"),
                  props={"discipline": doc.get("discipline"), "revision": doc.get("revision"),
                         "doc_type": doc.get("document_type")})
            pkg = (doc.get("linked_package") or "").strip()
            if pkg and f"package:{pkg}" in O.objects:
                O.link(did, f"package:{pkg}", "records")
            sec = (doc.get("section") or "").strip()
            if sec and f"section:{sec}" in O.objects:
                O.link(did, f"section:{sec}", "covers")
        for wf in d.get("workflows") or []:
            numb = (wf.get("document_number") or "").strip()
            did = f"doc:{numb}"
            if not numb or did not in O.objects:
                continue
            try:
                late = int(wf.get("days_late") or 0)
            except (TypeError, ValueError):
                late = 0
            if late > 0:
    ...
    for p in glob.glob(os.path.join(corpus, "**", "*.json"), recursive=True):
        if "_answer_key" in p:
            continue
        d = jload(p)
        if not isinstance(d, dict) or not isinstance(d.get("wbsElements"), list):
            continue
        cost_by_wbs = {}
        for c in d.get("costLines") or []:
            cost_by_wbs.setdefault(str(c.get("posid") or "").strip(), []).append(c)
        for w in d["wbsElements"]:
            posid = (w.get("posid") or "").strip()
            if not posid:
                continue
            wid = f"wbs:{posid}"
            budget = num(w.get("budget_inr"))
            lines = cost_by_wbs.get(posid, [])
            actual = sum(num(c.get("amount_inr")) for c in lines if str(c.get("value_type") or "").lower().startswith("actual"))
            commit = sum(num(c.get("amount_inr")) for c in lines if str(c.get("value_type") or "").lower().startswith("commit"))
            over = budget > 0 and (actual + commit) > budget
            O.obj(wid, "wbs", f"{posid} \u00b7 {str(w.get('description') or '')[:50]}",
                  status="OVER_BUDGET" if over else "",
                  props={"budget_inr": budget, "actual_inr": actual, "commitment_inr": commit,
                         "headroom_inr": round(budget - actual - commit)})
            if over:
                O.insight(wid, f"Committed + actual \u20b9{actual + commit:,.0f} exceeds budget \u20b9{budget:,.0f} - payment certification at risk.", "bad")
            pkg = (w.get("linked_package") or "").strip()
            if pkg and f"package:{pkg}" in O.objects:
                O.link(wid, f"package:{pkg}", "funds")
            sec = (w.get("linked_section") or "").strip()
            if sec and f"section:{sec}" in O.objects:
                O.link(wid, f"section:{sec}", "budgets")
